...
fw_configure_interface() {
	...
	fw__do_rules() {
		...
		lock /var/run/firewall-interface.lock
                ...
                if [ "$ifname" == "wlan0" ] || [ "$ifname" == "ath0" ] || [ "$ifname" == "ra0" ]; then
                fw $action $mode f ${chain}_ACCEPT ACCEPT $ { -o "br-lan" $onet }
                fw $action $mode f ${chain}_ACCEPT ACCEPT $ { -i "br-lan" $inet }
                fw $action $mode f ${chain}_DROP   DROP   $ { -o "br-lan" $onet }
                fw $action $mode f ${chain}_DROP   DROP   $ { -i "br-lan" $inet }
                fw $action $mode f ${chain}_REJECT reject $ { -o "br-lan" $onet }
                fw $action $mode f ${chain}_REJECT reject $ { -i "br-lan" $inet }
		else
		...
		fw $action $mode f input   ${chain}         $ { -i "$ifname" $inet }
                if [ "$ifname" == "wlan0" ] || [ "$ifname" == "ath0" ] || [ "$ifname" == "ra0" ]; then
			fw $action $mode f forward ${chain}_forward $ { -i "br-lan" $inet }
	                fw $action $mode n PREROUTING ${chain}_prerouting $ { -i "br-lan" $inet }
        	        fw $action $mode r PREROUTING ${chain}_notrack    $ { -i "br-lan" $inet }
			fw $action $mode n POSTROUTING ${chain}_nat       $ { -o "br-lan" $onet }
		else
